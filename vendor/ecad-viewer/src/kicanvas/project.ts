    ...
    *sch_in_order() {
    ...
    public get_first_page(kind: AssertType) {
        switch (kind) {
            case AssertType.SCH:
                return (
                    // KiCad PRISM feeds pre-flattened blobs where the top-level
                    // sheet is always named "root.kicad_sch"; prefer it so the
                    // project opens on the true root rather than whichever sheet
                    // the hierarchy heuristic sorts first.
                    (this._files_by_name.get("root.kicad_sch") as KicadSch) ??
