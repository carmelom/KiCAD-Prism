    ...
    constructor(
        ...
        Object.assign(
            ...
            parse_expr(
                ...
                P.collection("drawings", "table", T.item(Table, this)),
                P.collection(
                    "drawings",
                    "text",
...
export class TableCell {
    ...
    constructor(expr: Parseable) {
        /*
        (table_cell "Gain" (exclude_from_sim no) (at 127 80.01 0) (size 11.43 5.08)
          (margins 0.9525 0.9525 0.9525 0.9525) (span 1 1) (fill (type none))
          (effects (font (size 1.27 1.27)) (justify left top)) (uuid "..."))
        */
        Object.assign(
            this,
            parse_expr(
    ...
    /** Cell rectangle: (at) is the top-left corner, (size) is width/height. */
    get bbox(): BBox {
        return new BBox(
            this.at.position.x,
            this.at.position.y,
...
export class Table extends GraphicItem {
    column_count = 0;
    border?: { external?: boolean; header?: boolean; stroke?: Stroke };
    separators?: { rows?: boolean; cols?: boolean; stroke?: Stroke };
    column_widths: number[] = [];
    row_heights: number[] = [];
    cells: TableCell[] = [];

    constructor(expr: Parseable, parent?: LibSymbol | SchematicSymbol) {
        /* TODO: added in KiCad 8 */
        super(parent);
        const parsed = parse_expr(
            expr,
            P.start("table"),
            ...
            P.object(
                "cells",
                null,
                P.collection("items", "table_cell", T.item(TableCell)),
            ),
            P.pair("uuid", T.string),
        );
        Object.assign(this, parsed);
        this.cells = ((parsed["cells"] as { items?: TableCell[] })?.items ??
            []) as TableCell[];
    }

    /** Overall table rectangle: the union of all cell rectangles. */
    get bbox(): BBox {
        return BBox.combine(this.cells.map((c) => c.bbox));
    }
}
