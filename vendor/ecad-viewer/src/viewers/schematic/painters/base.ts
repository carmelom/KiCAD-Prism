...
/**
 * Map a KiCad stroke type to a canvas dash pattern (in world units, scaled by line
 * width). Returns undefined for solid/default/none so solid rendering is unchanged.
 */
function dash_for_stroke_type(
    type: string,
    width: number,
): number[] | undefined {
    const w = width || 0.1524; // KiCad default line width fallback
    switch (type) {
        case "dash":
            return [6 * w, 4 * w];
        case "dot":
            return [1.5 * w, 3 * w];
        case "dash_dot":
            return [6 * w, 3 * w, 1.5 * w, 3 * w];
        case "dash_dot_dot":
            return [6 * w, 3 * w, 1.5 * w, 3 * w, 1.5 * w, 3 * w];
        default:
            return undefined;
    }
}
...
export abstract class SchematicItemPainter extends ItemPainter {
    ...
    protected determine_stroke(
        ...
        return { width, color, dash: dash_for_stroke_type(stroke_type, width) };
    }
