...
class RectanglePainter extends SchematicItemPainter {
    ...
    #stroke(layer: ViewLayer, r: schematic_items.Rectangle, pts: Vec2[]) {
        const { width, color, dash } = this.determine_stroke(layer, r);
...
class PolylinePainter extends SchematicItemPainter {
    ...
    #stroke(layer: ViewLayer, pl: schematic_items.Polyline) {
        const { width, color, dash } = this.determine_stroke(layer, pl);

        if (!width || !color) {
            return;
        }

        this.gfx.line(new Polyline(pl.pts, width, color, dash));
    }
...
class BezierPainter extends SchematicItemPainter {
    ...
    #stroke(layer: ViewLayer, b: schematic_items.Bezier) {
        const { width, color, dash } = this.determine_stroke(layer, b);
        ...
        this.gfx.line(new Polyline(polyline, width, color, dash));
    }
...
class TablePainter extends SchematicItemPainter {
    ...
    paint(layer: ViewLayer, table: schematic_items.Table) {
        ...
        const stroke_color = table.border?.stroke?.color ?? this.theme.note;
        const stroke_width =
        ...
        if (table.border?.external) {
            this.gfx.line(Polyline.from_BBox(bbox, stroke_width, stroke_color));
        }

        // Internal row/column separators (skip the last edge; it coincides with
        // the external border).
        if (table.separators?.cols || table.separators?.rows) {
            const sep_color = table.separators.stroke?.color ?? stroke_color;
            const sep_width =
        ...
        for (const cell of table.cells) {
            const cell_bbox = cell.bbox;

            if (cell.fill && cell.fill.type !== "none" && cell.fill.color) {
                this.gfx.polygon(
                    Polygon.from_BBox(cell_bbox, cell.fill.color),
                );
            }
            ...
            // Reuse SchText for correct glyph shaping/units. We set text_pos
            // directly (top-left corner + margins) instead of apply_at, which
            // would override the cell's stored (justify left top) alignment.
            const schtext = new SchText(cell.text);
            schtext.apply_effects(cell.effects);

            const left = cell.margins?.[0] ?? 0;
            const top = cell.margins?.[1] ?? 0;
            const right = cell.margins?.[2] ?? 0;
            schtext.text_pos = new Vec2(
                (cell_bbox.x + left) * 10000,
                (cell_bbox.y + top) * 10000,
            );

            const font_color = cell.effects.font.color;
            schtext.attributes.color = font_color.is_transparent_black
                ? this.theme.note
                : font_color;

            const font = StrokeFont.default();

            // Wrap cell text to the cell's inner width, like KiCad's
            // SCH_TABLECELL (a SCH_TEXTBOX). Units here are internal (mm*10000),
            // matching attributes.size / stroke_width.
            const inner_width = (cell.size.x - left - right) * 10000;
            const wrapped =
                inner_width > 0
                    ? font.break_lines(
                          cell.text,
            ...
            this.gfx.state.push();
            font.draw(this.gfx, wrapped, schtext.text_pos, schtext.attributes);
            this.gfx.state.pop();
        }
    }
}
